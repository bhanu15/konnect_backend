...
class FeedbackService:
    ...
    @staticmethod
    def create_feedback(db: Session, feedback_data: FeedbackCreate, file: UploadFile | None):
    ...
    def list_feedbacks(db: Session, skip: int = 0, limit: int = 100):
        return FeedbackRepository.list(db, skip, limit)

    @staticmethod
    def update_feedback(db: Session, feedback_id: int, feedback_update: FeedbackUpdate, file: UploadFile | None):
        file_url = FeedbackService._upload_to_s3(file) if file else None
    ...
    @staticmethod
    def search_feedbacks_by_email(db: Session, email: str, skip: int = 0, limit: int = 100):
        return FeedbackRepository.search_by_email(db, email, skip, limit)
